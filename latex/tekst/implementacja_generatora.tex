\chapter{Implementacja generatora}
...
W niniejszym podrozdziale przedstawiono implementację oscylatora 
...
\cite{amd_vivado_keep}, że pętla kombinacyjna nie jest błędem projektowym, lecz zamierzoną strukturą
funkcjonalną. Bez tej deklaracji optymalizator mógłby usunąć lub uprościć logikę, traktując ją jako zbędną 
...
W niniejszym podrozdziale przedstawiono implementację generatora liczb losowych 
opartego na wielu instancjach oscylatora pierścieniowego oraz modułu próbkującego.
...
\begin{lstlisting}[language=VHDL,
caption={Moduł próbkujący (\texttt{ro\_sampler.vhdl})},
label={lst:ro_sampler}]
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
...
Listing~\ref{lst:ro_sampler} to moduł odpowiedzialny za próbkowanie i synchronizację ośmiobitowego
wektora sygnałów z oscylatorów pierścieniowych. Wejście \texttt{ro\_vec} jest asynchroniczne względem
zegara systemowego \texttt{clk}, dlatego nie może być bezpośrednio przekazane do dalszej logiki 
sekwencyjnej. W tym celu zastosowano dwustopniowy synchronizator zbudowany z rejestrów \texttt{f1} oraz 
\texttt{f2}, do których kolejne próbki sygnału są trafiają na narastającym zboczu zegara. Pierwszy stopień 
przechwytuje aktualny stan wejścia, drugi stopien ogranicza ryzyko propagacji metastabilności do dalszej 
części układu. Sygnał wyjściowy \texttt{ro\_sync} stanowi zsynchronizowaną z domeną zegarową wersję 
wektora \texttt{ro\_vec}, gotową do dalszej obróbki cyfrowej, na przykład zliczania, mieszania lub 
ekstrakcji bitów losowych. Dodatkowo użyto atrybutów \texttt{ASYNC\_REG}, \texttt{dont\_touch} oraz 
\texttt{KEEP}, aby poinformować narzędzia syntezy i implementacji, że rejestry te realizują ścieżkę 
synchronizacji sygnałów asynchronicznych i nie powinny zostać usunięte, scalone ani zoptymalizowane 
w sposób zmieniający ich funkcję\cite{amd_vivado_asyncreg}.
